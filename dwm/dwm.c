...
drawbar(Monitor *m)
{
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeTagsNorm]); //tag colors
//	drw_setscheme(drw, scheme[SchemeNorm]);
...
//			drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]); //mid bar active colors
			drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
//			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_setscheme(drw, scheme[SchemeInfoNorm]); //mid bar inactive colors
			drw_rect(drw, x, 0, w, bh, 1, 1);
		}
	}
